...
FontManager::FontManager() {
    if (!TTF_WasInit() && !TTF_Init()) {
        throw std::runtime_error(fmt::format("FontManager 错误: TTF_Init 失败：{}", SDL_GetError()));
    }
...
TTF_Font* FontManager::loadFont(std::string_view file_path, int point_size) {
    ...
    // 创建映射表的键
    FontKeyLookup key = {file_path, point_size};
    ...
    fonts_.emplace(FontKey{std::string(file_path), point_size},
                   std::unique_ptr<TTF_Font, SDLFontDeleter>(raw_font));
