...
AudioManager::AudioManager() {
    ...
    if ((Mix_Init(flags) & flags) != flags) {
        throw std::runtime_error(fmt::format("AudioManager 错误: Mix_Init 失败: {}", SDL_GetError()));
    }

    // SDL3打开音频设备的方法。默认值：44100 Hz，默认格式，2声道（立体声），2048采样块大小
    if (!Mix_OpenAudio(0, nullptr)) {
        Mix_Quit();  // 如果OpenAudio失败，先清理Mix_Init，再抛出异常
        throw std::runtime_error(fmt::format("AudioManager 错误: Mix_OpenAudio 失败: {}", SDL_GetError()));
    }
...
Mix_Chunk* AudioManager::loadSound(std::string_view file_path) {  // 首先检查缓存
...
Mix_Music* AudioManager::loadMusic(std::string_view file_path) {  // 首先检查缓存
