...
public:
     ...
     */
    explicit HealthComponent(int max_health = 1, std::chrono::duration<float> invincibility_duration = 2s);
    ...
    void setInvincible(std::chrono::duration<float> duration);
    ///< @brief 设置无敌状态持续时间。
    void setInvincibilityDuration(std::chrono::duration<float> duration) { invincibility_duration_ = duration; }

protected:
    // 核心循环函数
    void update(std::chrono::duration<float> delta_time, Context&) override;

private:
    int max_health_ = 1;                                         ///< @brief 最大生命值
    int current_health_ = 1;                                     ///< @brief 当前生命值
    bool is_invincible_ = false;                                 ///< @brief 是否处于无敌状态
    std::chrono::duration<float> invincibility_duration_{2.0f};  ///< @brief 受伤后无敌的总时长（秒）
    std::chrono::duration<float> invincibility_timer_{0.0f};     ///< @brief 无敌时间计时器（秒）
};
