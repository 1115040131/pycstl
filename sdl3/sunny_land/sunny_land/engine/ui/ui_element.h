...
public:
    ...
    virtual bool handleInput(Context& context);
    virtual void update(std::chrono::duration<float> delta_time, Context& context);
    virtual void render(Context& context);

    // --- 层次结构管理 ---
    void addChild(std::unique_ptr<UIElement> child);  ///< @brief 添加子元素
    std::unique_ptr<UIElement> removeChild(
        UIElement* child_ptr);  ///< @brief 将指定子元素从列表中移除，并返回其智能指针
    ...
    // --- 辅助方法 ---
    Rect getBounds() const;                            ///< @brief 获取(计算)元素的边界(屏幕坐标)
