...
JumpBehavior::JumpBehavior(float min_x, float max_x, glm::vec2 jump_vel,
                           std::chrono::duration<float> jump_interval)
    ...
    if (jump_interval_.count() <= 0.0f) {  // 确保跳跃间隔是正数
        spdlog::error("JumpBehavior: jump_interval ({}) 应为正数。已设置为 2.0f。", jump_interval.count());
        jump_interval_ = 2s;
    }
...
void JumpBehavior::update(std::chrono::duration<float> delta_time, AIComponent& ai_component) {  // 获取必要的组件
    ...
    if (is_on_ground) {
        ...
        if (jump_timer_ >= jump_interval_) {  // 时间到，准备跳跃
            ...
            physics_component->setVelocity({jump_vel_x, jump_vel_.y});      // 设置速度
