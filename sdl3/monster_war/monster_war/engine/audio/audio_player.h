 ...
 */
class AudioPlayer final : Noncopyable {
public:
    /**
     * @brief 构造函数，使用 ResourceManager 初始化。
     */
    explicit AudioPlayer(ResourceManager* resource_manager);
     ...
     * @return 音效正在播放的通道，出错时返回 std::nullopt。
     */
    std::optional<int> playSound(entt::id_type sound_id, int channel = -1);
     ...
     * @return 音效正在播放的通道，出错时返回 std::nullopt。
     */
    std::optional<int> playSound(entt::hashed_string hashed_path, int channel = -1);
     ...
     */
    bool playMusic(entt::id_type music_id, int loops = -1, std::chrono::milliseconds fade_in_ms = {});
     ...
     */
    void stopMusic(std::chrono::milliseconds fade_out_ms = {});
...
private:
    ResourceManager* resource_manager_;  ///< @brief 指向 ResourceManager 的非拥有指针，用于加载和管理音频资源。
