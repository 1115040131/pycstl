...
public:
    BasicEntityBuilder(LevelLoader& level_loader, Context& context, entt::registry& registry);
    virtual ~BasicEntityBuilder() = default;
    ...
    /// @brief 针对图片对象 (对象层)
    BasicEntityBuilder* configure(const nlohmann::json* object_json, const TileInfo* tile_info);

    /// @brief 针对瓦片 (瓦片层，相比“阳光岛”新增的功能)
    BasicEntityBuilder* configure(int index, const TileInfo* tile_info);

    virtual BasicEntityBuilder* build();                     ///< @brief 构建实体
    entt::entity getEntityID() const { return entity_id_; }  ///< @brief 获取实体ID（返回）
...
protected:
    ...
    const TileInfo* tile_info_ = nullptr;          ///< @brief 来自.tsj的瓦片数据
