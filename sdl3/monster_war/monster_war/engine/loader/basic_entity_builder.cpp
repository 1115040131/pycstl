...
BasicEntityBuilder::BasicEntityBuilder(LevelLoader& level_loader, Context& context, entt::registry& registry)
...
BasicEntityBuilder* BasicEntityBuilder::configure(const nlohmann::json* object_json, const TileInfo* tile_info) {
    reset();
...
BasicEntityBuilder* BasicEntityBuilder::configure(int index, const TileInfo* tile_info) {
    reset();
    if (!tile_info) {
...
void BasicEntityBuilder::buildBase() {
    ...
    if (object_json_ && object_json_->contains("name")) {
        ...
        registry_.emplace<NameComponent>(entity_id_, name_id, name);
...
void BasicEntityBuilder::buildSprite() {
    ...
    registry_.emplace<SpriteComponent>(entity_id_, tile_info_->sprite_);
}
...
void BasicEntityBuilder::buildTransform() {
    ...
    if (object_json_) {
        ...
        position_ =
            glm::vec2(position_.x, position_.y - dst_size_.y);  // 图片对象的position需要进行调整(左下角到左上角)
    ...
    if (index_ >= 0) {
        ...
        position_ = glm::vec2((index_ % map_size.x) * tile_size.x, (index_ / map_size.x) * tile_size.y);
    }
...
void BasicEntityBuilder::buildAnimation() {
    ...
    if (tile_info_ && tile_info_->animation_) {
        ...
        registry_.emplace<AnimationComponent>(entity_id_, std::move(animations), animation_id);
    }
}
...
template <typename T>
std::optional<T> BasicEntityBuilder::getTileProperty(const nlohmann::json& tile_json,
