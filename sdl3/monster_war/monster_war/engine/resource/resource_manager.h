...
public:
    ...
    // --- 统一资源访问接口 ---
    // -- Texture --
    SDL_Texture* loadTexture(entt::id_type id,
                             std::string_view file_path);  ///< @brief 载入纹理资源(通过id + 文件路径)
    SDL_Texture* loadTexture(entt::hashed_string str_hs);  ///< @brief 载入纹理资源(通过字符串哈希值)
    SDL_Texture* getTexture(entt::id_type id,
                            std::string_view file_path =
                                "");  ///< @brief 尝试获取已加载纹理的指针，如果未加载则尝试加载(通过id + 文件路径)
    SDL_Texture* getTexture(
        entt::hashed_string str_hs);  ///< @brief 尝试获取已加载纹理的指针，如果未加载则尝试加载(通过字符串哈希值)
    void unloadTexture(entt::id_type id);  ///< @brief 卸载指定的纹理资源
    glm::vec2 getTextureSize(entt::id_type id,
                             std::string_view file_path = "");  ///< @brief 获取指定纹理的尺寸(通过id + 文件路径)
    ...
    Mix_Chunk* loadSound(entt::id_type id,
                         std::string_view file_path);  ///< @brief 载入音效资源(通过id + 文件路径)
    Mix_Chunk* loadSound(entt::hashed_string str_hs);  ///< @brief 载入音效资源(通过字符串哈希值)
    Mix_Chunk* getSound(entt::id_type id,
                        std::string_view file_path =
                            "");  ///< @brief 尝试获取已加载音效的指针，如果未加载则尝试加载(通过id + 文件路径)
    Mix_Chunk* getSound(
        entt::hashed_string str_hs);  ///< @brief 尝试获取已加载音效的指针，如果未加载则尝试加载(通过字符串哈希值)
    void unloadSound(entt::id_type id);  ///< @brief 卸载指定的音效资源
    void clearSounds();                  ///< @brief 清空所有音效资源

    // -- Music --
    Mix_Music* loadMusic(entt::id_type id,
                         std::string_view file_path);  ///< @brief 载入音乐资源(通过id + 文件路径)
    Mix_Music* loadMusic(entt::hashed_string str_hs);  ///< @brief 载入音乐资源(通过字符串哈希值)
    Mix_Music* getMusic(entt::id_type id,
                        std::string_view file_path =
                            "");  ///< @brief 尝试获取已加载音乐的指针，如果未加载则尝试加载(通过id + 文件路径)
    Mix_Music* getMusic(
        entt::hashed_string str_hs);  ///< @brief 尝试获取已加载音乐的指针，如果未加载则尝试加载(通过字符串哈希值)
    void unloadMusic(entt::id_type id);  ///< @brief 卸载指定的音乐资源
    void clearMusic();                   ///< @brief 清空所有音乐资源

    // -- Fonts --
    TTF_Font* loadFont(entt::id_type id, int point_size,
                       std::string_view file_path);                  ///< @brief 载入字体资源(通过id + 文件路径)
    TTF_Font* loadFont(entt::hashed_string str_hs, int point_size);  ///< @brief 载入字体资源(通过字符串哈希值)
    TTF_Font* getFont(entt::id_type id, int point_size,
                      std::string_view file_path =
                          "");  ///< @brief 尝试获取已加载字体的指针，如果未加载则尝试加载(通过id + 文件路径)
    TTF_Font* getFont(
        entt::hashed_string str_hs,
        int point_size);  ///< @brief 尝试获取已加载字体的指针，如果未加载则尝试加载(通过字符串哈希值)
