...
DebugUISystem::DebugUISystem(entt::registry& registry, Context& context) : registry_(registry), context_(context) {
    context_.getDispatcher()
        .sink<UIPortraitHoverEnterEvent>()
        .connect<&DebugUISystem::onUIPortraitHoverEnterEvent>(this);
    context_.getDispatcher()
        .sink<UIPortraitHoverLeaveEvent>()
        .connect<&DebugUISystem::onUIPortraitHoverLeaveEvent>(this);
}

DebugUISystem::~DebugUISystem() { context_.getDispatcher().disconnect(this); }
...
void DebugUISystem::updateTitle(TitleScene& title_scene) {
    beginFrame();
    renderTitleLogo();
...
void DebugUISystem::updateLevelClear(LevelClearScene& level_clear_scene) {
    beginFrame();
...
void DebugUISystem::updateEnd(EndScene& end_scene) {
    beginFrame();
...
void DebugUISystem::renderHoveredPortrait() {  // 确定鼠标悬浮的单位肖像存在
    if (hovered_portrait_ == entt::null) {
        return;
    }

    // 角色名称不是一个实体，需要从蓝图中获取数据
    const auto& session_data = registry_.ctx().get<std::shared_ptr<SessionData>>();
    const auto& blueprint_mgr = registry_.ctx().get<std::shared_ptr<BlueprintManager>>();
    ...
    // 计算等级和稀有度对属性的影响
    const auto hp = statModify(stats.hp_, unit_data.level_, unit_data.rarity_);
    const auto atk = statModify(stats.atk_, unit_data.level_, unit_data.rarity_);
    const auto def = statModify(stats.def_, unit_data.level_, unit_data.rarity_);
...
void DebugUISystem::renderSelectedUnit() {
    ...
    auto available_cost = registry_.ctx().get<GameStats&>().cost_;
    ...
    if (ImGui::Button("升级")) {
        context_.getDispatcher().enqueue<UpgradeUnitEvent>(entity, player.cost_);
    }
    ImGui::SameLine();
    ...
    if (ImGui::Button("撤退")) {
        context_.getDispatcher().enqueue<RetreatEvent>(entity, return_cost);
    }
    ImGui::SameLine();
    ImGui::Text("快捷键 R: COST返还: %d", return_cost);

    // 技能显示与交互
    if (auto skill = registry_.try_get<SkillComponent>(entity)) {
        // 如果技能准备就绪，则按钮可用（激活技能），否则按钮不可用
        auto ready = registry_.all_of<SkillReadyTag>(entity);
        ...
        if (ImGui::Button(skill->name_.c_str())) {
            // 激活技能
            context_.getDispatcher().enqueue<SkillActiveEvent>(entity);
        }
        ImGui::EndDisabled();
        ImGui::SameLine();
        // 如果技能激活中，则显示"剩余时间"或"被动技能激活中"
        if (registry_.all_of<SkillActiveTag>(entity)) {
            if (registry_.all_of<PassiveSkillTag>(entity)) {
                ImGui::Text("被动技能激活中");
            } else {
                ImGui::Text("激活中，剩余时间: %.1f 秒", (skill->duration_ - skill->duration_timer_).count());
            }
        } else {  // 否则显示冷却时间
            ImGui::Text("快捷键 S: ");
            ImGui::SameLine();
            if (registry_.all_of<SkillReadyTag>(entity)) {
                ImGui::Text("技能准备就绪");
            } else {
                // 用进度条显示冷却时间百分比
...
void DebugUISystem::renderInfoUI() {
    ...
    // 获取关卡相关数据
    const auto& game_stats = registry_.ctx().get<GameStats&>();
    const auto& waves = registry_.ctx().get<Waves&>();
    const auto& session_data = registry_.ctx().get<std::shared_ptr<SessionData>>();
    // 显示
    ...
    if (waves.waves_.size() > 0) {
        ImGui::Text("下一波时间: %d", static_cast<int>(waves.next_wave_count_down_.count()));
    }
    ImGui::SameLine();
...
void DebugUISystem::renderSettingUI() {
    ...
    if (game_state.isPaused()) {  // 如果游戏暂停，则显示“继续游戏”按钮，快捷键 P
        if (ImGui::Button("继续游戏")) {
            game_state.setState(State::Playing);
        }
    } else {  // 如果游戏运行中，则显示“暂停游戏”按钮，快捷键也是 P
        if (ImGui::Button("暂停游戏")) {
            game_state.setState(State::Paused);
        }
    }
    ImGui::SameLine();
    if (ImGui::Button("重新开始")) {
        context_.getDispatcher().enqueue<RestartEvent>();
    }
    if (ImGui::Button("返回标题")) {
        context_.getDispatcher().enqueue<BackToTitleEvent>();
    }
    ImGui::SameLine();
    if (ImGui::Button("保存")) {
        context_.getDispatcher().enqueue<SaveEvent>();
    }
    ImGui::Separator();
...
void DebugUISystem::renderDebugUI() {
    ...
    if (ImGui::Button("通关")) {
        context_.getDispatcher().enqueue<LevelClearEvent>();
    }
    // TODO: 未来可按需添加其他调试工具
    ImGui::End();
}
...
void DebugUISystem::renderLevelClearTable(LevelClearScene& level_clear_scene) {
...
void DebugUISystem::renderLevelClearButtons(LevelClearScene& level_clear_scene) {
...
void DebugUISystem::renderEndText(EndScene& end_scene) {
...
void DebugUISystem::renderEndButtons(EndScene& end_scene) {
...
void DebugUISystem::renderTitleButtons(TitleScene& title_scene) {
...
void DebugUISystem::renderUnitTable() {
    ...
    const auto ui_config = registry_.ctx().get<std::shared_ptr<UIConfig>>();
    ...
    if (ImGuiTableSortSpecs* sort_specs = ImGui::TableGetSortSpecs()) {
        if (sort_specs->SpecsDirty && !unit_data_list.empty()) {
            ...
            // 执行排序
            std::stable_sort(
                unit_data_list.begin(), unit_data_list.end(), [&](const UnitData* lhs, const UnitData* rhs) {
                    // 获取职业蓝图（用于获取属性）
                    ...
                    switch (col) {
                        ...
                        case 5: {  // COST
                            ...
                            const int cost_l =
                                static_cast<int>(std::round(statModify(pcb_l.player_.cost_, 1, lhs->rarity_)));
                            const int cost_r =
                                static_cast<int>(std::round(statModify(pcb_r.player_.cost_, 1, rhs->rarity_)));
                            delta = (cost_l < cost_r) ? -1 : (cost_l > cost_r ? 1 : 0);
                            break;
                        }
                        case 6: {  // 生命值
                            const int hp_l = static_cast<int>(
                                std::round(statModify(pcb_l.stats_.hp_, lhs->level_, lhs->rarity_)));
                            const int hp_r = static_cast<int>(
                                std::round(statModify(pcb_r.stats_.hp_, rhs->level_, rhs->rarity_)));
                            delta = (hp_l < hp_r) ? -1 : (hp_l > hp_r ? 1 : 0);
                            break;
                        }
                        case 7: {  // 攻击力
                            const int atk_l = static_cast<int>(
                                std::round(statModify(pcb_l.stats_.atk_, lhs->level_, lhs->rarity_)));
                            const int atk_r = static_cast<int>(
                                std::round(statModify(pcb_r.stats_.atk_, rhs->level_, rhs->rarity_)));
                            delta = (atk_l < atk_r) ? -1 : (atk_l > atk_r ? 1 : 0);
                            break;
                        }
                        case 8: {  // 防御力
                            const int def_l = static_cast<int>(
                                std::round(statModify(pcb_l.stats_.def_, lhs->level_, lhs->rarity_)));
                            const int def_r = static_cast<int>(
                                std::round(statModify(pcb_r.stats_.def_, rhs->level_, rhs->rarity_)));
                        ...
                        case 10: {  // 攻击间隔
                            const auto ai_l = pcb_l.stats_.atk_interval_;
                            const auto ai_r = pcb_r.stats_.atk_interval_;
                            delta = (ai_l < ai_r) ? -1 : (ai_l > ai_r ? 1 : 0);
                            break;
                        }
                        case 11: {  // 阻挡数量
                            delta = (pcb_l.player_.block_ < pcb_r.player_.block_)
                                        ? -1
                                        : (pcb_l.player_.block_ > pcb_r.player_.block_ ? 1 : 0);
                            break;
                        }
                        case 12: {  // 技能
    ...
    for (const auto& unit : unit_data_list) {
        ...
        const auto& skill_blueprint =
            blueprint_manager->getSkillBlueprint(player_class_blueprint.player_.skill_id_);
        const auto& stats = player_class_blueprint.stats_;
        const auto hp = statModify(stats.hp_, unit->level_, unit->rarity_);
        const auto atk = statModify(stats.atk_, unit->level_, unit->rarity_);
        const auto def = statModify(stats.def_, unit->level_, unit->rarity_);
        const auto cost = statModify(player_class_blueprint.player_.cost_, 1, unit->rarity_);
        std::string type = player_class_blueprint.player_.type_ == PlayerType::MELEE    ? "近战"
                           : player_class_blueprint.player_.type_ == PlayerType::RANGED ? "远程"
                           : player_class_blueprint.player_.type_ == PlayerType::MIXED  ? "混合"
                                                                                        : "未知";

        // 获取头像信息
        const auto& portrait_image = ui_config->getPortrait(unit->name_id_);
        auto portrait_texture = context_.getResourceManager().getTexture(portrait_image.getTextureId(),
                                                                         portrait_image.getTexturePath());
        auto portrait_rect = portrait_image.getSourceRect();  // 源矩形的区域
        auto sprite_sheet_size =
            context_.getResourceManager().getTextureSize(portrait_image.getTextureId());  // 获取精灵图的尺寸
        ...
        ImGui::Text("%.2f", player_class_blueprint.stats_.atk_interval_.count());
        ...
        if (ImGui::Button(
                button_text.c_str())) {  // 如果没有PushID，默认会以Button中显示参数作为ID，那会出现重复ID
...
void DebugUISystem::onUIPortraitHoverEnterEvent(const UIPortraitHoverEnterEvent& event) {
    hovered_portrait_ = event.name_id_;
}

void DebugUISystem::onUIPortraitHoverLeaveEvent() { hovered_portrait_ = entt::null; }
