...
void ImGuiWindow1() {
    ...
    if (ImGui::SliderFloat("音量", &volume_value, 0.0f, 1.0f)) {
        spdlog::info("音量设置为: {}", volume_value);
    }
    ImGui::End();
}
...
void ImGuiWindow2(SDL_Renderer* renderer) {
    ImGui::Begin("窗口2");
    // 显示图片
    auto texture = IMG_LoadTexture(renderer, "sdl3/monster_war/example/assets/textures/Buildings/Castle.png");
    if (texture) {
...
void ImGuiOptionalSettings() {
    ...
    ImFont* font = io.Fonts->AddFontFromFileTTF(
        "sdl3/monster_war/example/assets/fonts/VonwaonBitmap-16px.ttf",  // 字体文件路径
        16.0f,                                                           // 字体大小
        nullptr,                                                         // 字体配置参数
...
int main() {
    ...
    SDL_CreateWindowAndRenderer("Hello World!", 1280, 800, 0, &window, &renderer);
    if (!window || !renderer) {
