...
from venv_utils import VenvName, run_venv, venv
...
def chat_run(args: list[str]) -> None:
...
# chat 相关数据
DATA_DIRECTORY = f'{root_path}/chat/server/mysql/data'
LOG_DIRECTORY = f'{root_path}/chat/server/mysql/logs'


def run_target(name: str, args: list[str] | None = None) -> Any:
    """按名字执行其他 target, 供 target 之间相互复用"""
    return TARGETS[name](args or [])
...
TARGETS: dict[str, Callable[[list[str]], Any]] = {
    ...
    "chat_mysql_server": lambda args: (
        # 创建目录并设置访问权限
        setup_directory(DATA_DIRECTORY),
        setup_directory(LOG_DIRECTORY),
        ...
        run_docker(
            ...
            args=[f'-v {root_path}/chat/server/mysql/config/my.cnf:/etc/my.cnf',
                  ...
                  f'-v {DATA_DIRECTORY}:/var/lib/mysql',
                  f'-v {LOG_DIRECTORY}:/logs',
    ...
    "chat_prepare_start": lambda args: (
        run_target("chat_redis_server"),
        run_target("chat_mysql_server"),
    ),
    ...
    "chat_prepare": lambda args: (
        run_target("chat_prepare_start"),
        run_target("chat_prepare_wait"),
    ),
    "chat_clear": lambda args: (
        ...
        run_cmd(f'sudo rm -rf {DATA_DIRECTORY}'),
        run_cmd(f'sudo rm -rf {LOG_DIRECTORY}'),
    ),
    ...
    ######################### build for nn #########################
    # download dataset
    "fineweb": lambda args: run_venv(VenvName.NN, f'python {root_path / "nn/nanoGPT/fineweb.py"}', args),
    "hellaswag": lambda args: run_venv(VenvName.NN, f'python {root_path / "nn/nanoGPT/hellaswag.py"}', args),
    # train_gpt
    "bigram": lambda args: run_venv(VenvName.NN, f'python {root_path / "nn/nanoGPT/bigram.py"}', args),
    "gpt": lambda args: run_venv(VenvName.NN, f'python {root_path / "nn/nanoGPT/gpt.py"}', args),
    # train_gpt2
    "train_gpt2": lambda args: run_venv(VenvName.NN, f'python {root_path / "nn/nanoGPT/train_gpt2.py"}', args),
    "train_gpt2_ddp": lambda args: run_venv(VenvName.NN, f'torchrun --standalone --nproc_per_node=8 {root_path / "nn/nanoGPT/train_gpt2.py"}', args),
    # micrograd
