...
TEST(ReactionTest, CalcTest) {
    ...
    auto dds =
        reaction::calc([](int aa, double dsds) { return std::to_string(aa) + std::to_string(dsds); }, a, ds);
    EXPECT_DOUBLE_EQ(ds.get(), 4.14);
    EXPECT_EQ(dds.get(), "14.140000");

    a.value(2);
    ASSERT_DOUBLE_EQ(ds.get(), 5.14);
...
TEST(ReactionTest, SelfDependencyTest) {
    auto a = reaction::var(1);
...
TEST(ReactionTest, CycleDependencyTest) {
    auto a = reaction::var(1);
    auto b = reaction::var(2);
...
TEST(ReactionTest, RepeatDependencyTest) {
    // ds → A, ds → a, A → a
    auto a = reaction::var(1);
    auto b = reaction::var(2);
...
TEST(ReactionTest, RepeatDependency2Test) {
...
TEST(ReactionTest, RepeatDependency3Test) {
...
// TEST(ReactionTest, CloseStraTest) {
...
//     {
//         auto dsA =
//             reaction::calc<reaction::ChangeTrig, reaction::CloseStra>([](int aa) { return aa; },
//             a).setName("dsA");
...
// TEST(ReactionTest, KeepStraTest) {
//     auto a = reaction::var(1).setName("a");
...
// TEST(ReactionTest, LastStraTest) {
//     auto a = reaction::var(1).setName("a");
...
//     {
//         auto dsA =
//             reaction::calc<reaction::ChangeTrig, reaction::LastStra>([](int aa) { return aa; },
//             a).setName("dsA");
...
TEST(DISABLED_ReactionTest, StressTest) {
    ...
    auto layer7 =
        calc([](const std::vector<int>& vec) { return std::accumulate(vec.begin(), vec.end(), 0); }, layer6);

    // Layer 8: Create a ProcessedData object with checksum and info
    auto layer8 = calc([](int sum) { return ProcessedData{"ProcessedData", static_cast<int>(sum)}; }, layer7);

    // Layer 9: Combine info and checksum into a string
    auto layer9 =
        calc([](const ProcessedData& calc) { return calc.info + "|" + std::to_string(calc.checksum); }, layer8);
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        ...
        if (i % 10000 == 0 && finalLayer.get() == expected) {
            ...
            std::cout << "Progress: " << i << "/" << ITERATIONS << " (" << dur.count() << "ms)\n";
        }
    }
    ...
    std::cout << "=== Stress Test Results ===\n"
              ...
              << "Avg time per update: " << duration.count() / static_cast<double>(ITERATIONS) << "ms\n";
}
